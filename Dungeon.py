...
def roundm(n, m):
    '''Rounds a number n to the nearest multiple of m
       If n is iterable , apply rounding to each element'''
...
def slope(p0, p1):
    '''Slope calculate between points'''
    try:
...
def collide_and_scatter_rooms(left, right):
    '''
    If the rooms collide, apply repulsion to the right
    Stop the right room's velocity if there's no collision
...
def collide_with_voids(left, right):
    '''Collision check with rooms marked as voids
    '''
...
if __name__ == '__main__':
    ...
    from pygame.locals import QUIT, K_ESCAPE, K_SPACE, K_r
